...
`;
  ...
  public async addDomain(domain: string, port: number): Promise<void> {
    try {
      this.addHostsEntry(domain);
      this.updateNginxConfig(domain, port);
  ...
  private async ensureContainerRunning(): Promise<void> {
    try {
      ...
      if (containerExists) {
        ...
        // Start the container with port 80 exposed
  ...
  private updateNginxConfig(domain: string, port: number): void {
    const configPath = join(this.configDir, 'conf.d', `${domain}.conf`);
    const config = `
server {
    listen 80;
