...
export default class Init extends BaseCommand {
  ...
  async run(): Promise<void> {
    ...
    // Normalize domain if it was set via interactive mode
    if (flags.domain && typeof flags.domain === 'string') {
      flags.domain = this.normalizeDomain(flags.domain);
    }
  ...
  private async addMultisiteSubdomainHelper(containerName: string, flags: Record<string, unknown>): Promise<void> {
    try {
      const networkDomain = flags.domain as string;

      // Create the PHP helper code with JavaScript injection
      const helperCode = this.getMultisiteHelperCode(networkDomain);

      // Create the helper as a mu-plugin (must-use plugin)
      const tempHelperPath = join(tmpdir(), 'wp-spin-multisite-helper.php');
      fs.writeFileSync(tempHelperPath, helperCode);

      // Copy to mu-plugins directory in container
      execSync(`docker exec ${containerName} mkdir -p /var/www/html/wp-content/mu-plugins`, { stdio: 'pipe' });
      execSync(`docker cp ${tempHelperPath} ${containerName}:/var/www/html/wp-content/mu-plugins/wp-spin-multisite-helper.php`, { stdio: 'pipe' });

      // Clean up temp file
      fs.unlinkSync(tempHelperPath);

      console.log(chalk.blue('✓ Added multisite subdomain helper for local development'));
    } catch (error) {
      console.warn('Warning: Could not install multisite helper plugin');
...
require_once ABSPATH . 'wp-settings.php';`;
  ...
  private getMultisiteHelperCode(networkDomain: string): string {
    return `<?php
/**
 * WP-Spin Multisite Helper
 * Auto-generated by wp-spin init for subdomain multisite local development
 * Only shows notices in local wp-spin environments
 */

// Only run on multisite subdomain installs
if (is_multisite() && is_subdomain_install()) {
    add_action('admin_footer', 'wp_spin_inject_multisite_notice');
    add_action('network_admin_footer', 'wp_spin_inject_multisite_notice');
}

function wp_spin_inject_multisite_notice() {
    // Only show on the "Add New Site" page
    $screen = get_current_screen();
    if (!$screen || $screen->id !== 'site-new-network') {
        return;
    }

    // Multiple checks to ensure this is a local wp-spin environment
    if (!wp_spin_is_local_environment()) {
        return;
    }
    ...
    function addWpSpinNotice() {
        // Check if notice already exists to prevent duplicates
        if (document.querySelector('#wp-spin-multisite-notice')) {
            return;
        }

        var noticeHtml = '<div id="wp-spin-multisite-notice" class="notice notice-info" style="border-left-color: #0073aa; margin: 20px 0;">' +
            '<p><strong>🚀 wp-spin Local Development Notice:</strong></p>' +
            '<p>After creating a new subdomain site, add this line to your hosts file:</p>' +
            '<pre style="background: #f1f1f1; padding: 10px; font-family: monospace; border-radius: 3px; margin: 10px 0; border-left: 3px solid #0073aa;">127.0.0.1 <strong>newsite</strong>.<?php echo $network_domain; ?></pre>' +
            '<p><strong>Hosts file locations:</strong><br>' +
            '• <strong>Mac/Linux:</strong> <code>/etc/hosts</code><br>' +
            '• <strong>Windows:</strong> <code>C:\\Windows\\System32\\drivers\\etc\\hosts</code></p>' +
            '<p><strong>Command line (Mac/Linux):</strong></p>' +
            '<pre style="background: #f9f9f9; padding: 8px; font-family: monospace; font-size: 12px;">echo "127.0.0.1 <strong>newsite</strong>.<?php echo $network_domain; ?>" | sudo tee -a /etc/hosts</pre>' +
            '<p><em>Replace <strong>newsite</strong> with your actual subdomain name.</em></p>' +
            '<p style="font-size: 12px; color: #666;"><em>This notice only appears in local wp-spin environments.</em></p>' +
            '</div>';

        // Try multiple selectors to find the right place to insert
        var insertTarget = document.querySelector('.wrap h1') ||
                          document.querySelector('#wpbody-content h1') ||
                          document.querySelector('.wrap .page-title-action').parentNode ||
                          document.querySelector('#wpbody-content');

        if (insertTarget) {
            insertTarget.insertAdjacentHTML('afterend', noticeHtml);
        }
    }

    // Try immediately and also after DOM/jQuery ready
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', addWpSpinNotice);
    } else {
        addWpSpinNotice();
    }
...
function wp_spin_is_local_environment() {
    // Multiple checks to ensure this is local
    return (
        // Check for wp-spin environment variable
        (getenv('WP_SPIN_ENV') === 'true') ||

        // Check for local domains
        (in_array($_SERVER['HTTP_HOST'] ?? '', [
            'localhost',
        ]) ||
        preg_match('/\\.(test|local|dev)$/', $_SERVER['HTTP_HOST'] ?? '')) ||

        // Check for wp-spin marker files (relative to WordPress root)
        (file_exists(ABSPATH . '../docker-compose.yml') &&
         file_exists(ABSPATH . '../.wp-spin')) ||

        // Check for local IP ranges
        (isset($_SERVER['SERVER_ADDR']) &&
         (strpos($_SERVER['SERVER_ADDR'], '127.0.0.1') === 0 ||
          strpos($_SERVER['SERVER_ADDR'], '::1') === 0))
    );
}
`;
  }
...
`;
  ...
  private async setupMultisite(containerName: string, siteName: string, flags: Record<string, unknown>): Promise<void> {
    ...
    try {
      ...
      // Add multisite subdomain helper for local development
      if (flags['multisite-type'] === 'subdomain') {
        await this.addMultisiteSubdomainHelper(containerName, flags);
      }
    } catch (error) {
...
upload_max_filesize = 64M`;
  ...
  private async updateSiteUrls(containerName: string, flags: Record<string, unknown>): Promise<void> {
    if (flags.domain === undefined) {
      // No custom domain - use localhost with port
      execSync(
      ...
      // Custom domain provided - set URLs in database to match domain
      const protocol = flags.ssl ? 'https' : 'http';
      const domainUrl = `${protocol}://${flags.domain}`;

      execSync(
        `docker exec ${containerName} sh -c "cd /var/www/html && php -d memory_limit=512M /usr/local/bin/wp option update home '${domainUrl}' --allow-root"`,
        { stdio: 'inherit' }
      );
      execSync(
        `docker exec ${containerName} sh -c "cd /var/www/html && php -d memory_limit=512M /usr/local/bin/wp option update siteurl '${domainUrl}' --allow-root"`,
        { stdio: 'inherit' }
      );

      this.logDebug(`WordPress URLs configured as: ${domainUrl}`);
    }
  }
